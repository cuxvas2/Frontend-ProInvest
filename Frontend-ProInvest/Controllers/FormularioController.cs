...
namespace Frontend_ProInvest.Controllers
{
    public class FormularioController : Controller
    {
        ...
        [HttpPost]
        public async Task<JsonResult> ObtenerFirma(string folioInversion)
        {
            try
            {
                ...
                {
                    return Json(new { firma = contrato.InformacionContrato.Contrato });
                }
                else
                {
                    {
                        return Json(new { error = "No se pudo recuperar la firma" });
                    }
                }
            }
            catch(Exception ex)
